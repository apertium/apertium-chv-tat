...
# and then compile:
cd ../../
sed -i 's_^<e>       <re>\[№.*$_<!--&-->_' apertium-chv-tat.chv-tat.dix
make
cd testvoc/standard/
...
MONODIX=../../../.deps/chv-tat.automorf.trimmed

echo "==Chuvash->Tatar==========================="

expand_monodix |
bash inconsistency.sh chv-tat > $TMPDIR/chv-tat.testvoc
bash inconsistency-summary.sh $TMPDIR/chv-tat.testvoc chv-tat
